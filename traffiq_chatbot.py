...
def chat_fn(message, history, show_sql, show_raw_data):
    ...
    try:
        ...
        if resp.status_code != 200:
            ...
            if show_sql and data.get('sql_query'):
                assistant_msg += f"\n\n**SQL Query:**\n```sql\n{data['sql_query']}\n```"

            # Add raw data if available
            if show_raw_data and data.get('data'):
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        show_sql = gr.Checkbox(label="Show SQL query", value=True)
        show_raw_data = gr.Checkbox(label="Show raw data", value=True)

    clear = gr.Button("Clear Chat")

    # Event handlers
    msg.submit(chat_fn, [msg, chatbot, show_sql, show_raw_data], chatbot).then(
        lambda: "", None, msg  # Clear input after submit
    )
    submit.click(chat_fn, [msg, chatbot, show_sql, show_raw_data], chatbot).then(
        lambda: "", None, msg
    )
